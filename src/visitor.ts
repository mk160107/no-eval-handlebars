...
import { initData, isBlock, isDecorator, noop, toDecoratorOptions, transformLiteralToPath } from './utils';
...
export class ElasticHandlebarsVisitor extends Handlebars.Visitor {
    ...
    PathExpression(path: hbs.AST.PathExpression) {
        const blockParamId = !path.depth && !AST.helpers.scopedId(path) && this.blockParamIndex(path.parts[0]);
    ...
    private processDecorator(
        ...
        const result = this.container.lookupProperty<DecoratorDelegate>(this.container.decorators, options.name)(
            prog,
            props,
            this.container,
            options,
    ...
    private processHelperNode(node: ProcessableNodeWithPathParts) {
        ...
        if (this.compileOptions.knownHelpers && this.compileOptions.knownHelpers[name]) {
            ...
            throw new Handlebars.Exception('You specified knownHelpersOnly, but used the unknown helper ' + name, node);
        } else {
    ...
    private invokePartial(partial: hbs.AST.PartialStatement | hbs.AST.PartialBlockStatement) {
        ...
        if (params.length > 1) {
            throw new Handlebars.Exception(`Unsupported number of partial arguments: ${params.length}`, partial);
        }
        ...
        const partialTemplate: Template | undefined =
            ...
            Handlebars.VM.resolvePartial(undefined, undefined, options as Handlebars.ResolvePartialOptions);
    ...
    private processAmbiguousNode(node: ProcessableNodeWithPathParts) {
        ...
        if (helperFn === undefined) {
            if (this.compileOptions.strict) {
                ...
                helperFn =
                    helper.context != null ? this.container.lookupProperty(helper.context, name) : helper.context;
                if (helperFn == null) helperFn = this.container.hooks.helperMissing;
            }
        }

        const helperResult =
            typeof helperFn === 'function' ? helperFn.call(helper.context, ...helper.params, helper.options) : helperFn;
