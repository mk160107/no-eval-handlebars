    ...
    forEachCompileFunctionName((compileName) => {
        ...
        describe(`#${compileName}`, () => {
            it('should fail with invalid input', () => {
                expect(function () {
                    compile(null);
                }).toThrow(`You must pass a string or Handlebars AST to Handlebars.${compileName}. You passed null`);
            ...
            it('should not modify the options.knownHelpers property(GH-1327)', () => {
                ...
                compile('{{#each data}}{{@index}}:{{a}} {{/each}}', options)();
                expect(JSON.stringify(options, null, 2)).toEqual(JSON.stringify({ knownHelpers: {} }, null, 2));
            });
        });
    });
});
