...
describe('data', () => {
    ...
    it('deep @foo triggers automatic top-level data', () => {
        ...
        expectTemplate('{{#let world="world"}}{{#if foo}}{{#if foo}}Hello {{@world}}{{/if}}{{/if}}{{/let}}')
            .withInput({ foo: true })
            .withHelpers(helpers)
    ...
    it('data is inherited downstream', () => {
        expectTemplate('{{#let foo=1 bar=2}}{{#let foo=bar.baz}}{{@bar}}{{@foo}}{{/let}}{{@foo}}{{/let}}')
    ...
    describe('nesting', () => {
        it('the root context can be looked up via @root', () => {
            expectTemplate('{{#helper}}{{#helper}}{{@./depth}} {{@../depth}} {{@../../depth}}{{/helper}}{{/helper}}')
                .withInput({ foo: 'hello' })
