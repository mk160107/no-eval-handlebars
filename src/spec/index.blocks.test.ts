...
describe('blocks', () => {
    ...
    it('block with deep nested complex lookup', () => {
        expectTemplate('{{#outer}}Goodbye {{#inner}}cruel {{../sibling}} {{../../omg}}{{/inner}}{{/outer}}')
            .withInput({
                omg: 'OMG!',
    ...
    describe('inverted sections', () => {
        it('inverted sections with unset value', () => {
            expectTemplate('{{#goodbyes}}{{this}}{{/goodbyes}}{{^goodbyes}}Right On!{{/goodbyes}}').toCompileTo(
                'Right On!',
            );
        });
        ...
        it('chained inverted sections', () => {
            ...
            expectTemplate('{{#people}}{{name}}{{else if nothere}}fail{{else unless nothere}}{{none}}{{/people}}')
                .withInput({ none: 'No people' })
                .toCompileTo('No people');
    ...
    describe('decorators', () => {
        ...
        it('should support nested decorators', () => {
            expectTemplate('{{#helper}}{{#*decorator}}{{#*nested}}suc{{/nested}}cess{{/decorator}}{{/helper}}')
                .withHelper('helper', function (options: HelperOptions) {
                    return (options.fn as any).run;
                })
                .withDecorators({
        ...
        it('should apply multiple decorators', () => {
            expectTemplate('{{#helper}}{{#*decorator}}suc{{/decorator}}{{#*decorator}}cess{{/decorator}}{{/helper}}')
                .withHelper('helper', function (options: HelperOptions) {
                    return (options.fn as any).run;
                })
                .withDecorator('decorator', function (fn, props, container, options) {
