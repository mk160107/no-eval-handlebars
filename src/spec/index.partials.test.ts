...
describe('partials', () => {
    ...
    it('partials with string context', () => {
        expectTemplate('Dudes: {{>dude "dudes"}}').withPartial('dude', '{{.}}').toCompileTo('Dudes: dudes');
    });
    ...
    // We do not support that mode with `@elastic/handlebars`, so there's no need to test it
    ...
    describe('partial blocks', () => {
        ...
        it('should not use partial block if partial exists', () => {
            expectTemplate('{{#> dude}}fail{{/dude}}').withPartials({ dude: 'success' }).toCompileTo('success');
        });
    ...
    describe('inline partials', () => {
        it('should define inline partials for template', () => {
            expectTemplate('{{#*inline "myPartial"}}success{{/inline}}{{> myPartial}}').toCompileTo('success');
        });
        ...
        it('should define inline partials for block', () => {
            expectTemplate('{{#with .}}{{#*inline "myPartial"}}success{{/inline}}{{> myPartial}}{{/with}}').toCompileTo(
                'success',
            );

            expectTemplate('{{#with .}}{{#*inline "myPartial"}}success{{/inline}}{{/with}}{{> myPartial}}').toThrow(
                /myPartial could not/,
            );
        });
